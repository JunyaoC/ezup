...
def register_device(
    ...
    *,
    ...
    """POST /v1/device with the ADMIN bearer -- device minting is admin-gated.

    Standalone rather than an HttpProvider method because it authenticates with
    the admin token, not the machine's device token, and is used before any
    device token exists. The device secret is generated by the caller; only its
    hash is sent, so the server can authenticate the device later without ever
    holding a token that could publish or read as it.
    """
    body = json.dumps(
        {"name": name, "email": email, "token_sha256": token_sha256}
    ).encode("utf-8")
    request = urllib.request.Request(
        base_url.rstrip("/") + "/v1/device",
        data=body,
        method="POST",
        headers={
    ...
    try:
        ...
        detail = _error_detail(exc)
        hint = (
            "  (the admin token is wrong, or this IP is briefly locked out after "
            "failed attempts -- wait a minute and retry)"
            if exc.code in (401, 403)
            else ""
        )
        raise TransportError(
            f"device registration failed: HTTP {exc.code} {exc.reason}{detail}{hint}",
            status=exc.code,
            url=request.full_url,
        ) from exc
    except (urllib.error.URLError, TimeoutError, OSError) as exc:
        raise TransportError(f"device registration failed: {exc}") from exc
